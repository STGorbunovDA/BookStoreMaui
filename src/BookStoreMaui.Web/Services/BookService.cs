    ...
    public class BookService : IBookService
    {
        ...
        public async Task<BookListDto[]> GetPopularBooksAsync(int count, string? genreSlug = null)
        {
            ...
            if (books.Length < count)
            {
                ...
                // У нас нет {количества} книг в этом жанре
                // в этом случае мы получим случайные книги из других жанров
        ...
        public async Task<BookDetailsDto> GetBookAsync(int bookId)
        {
            ...
            var book = await context.Books.Where(b => b.Id == bookId)
                                          .Select(b => new BookDetailsDto(b.Id, b.Title, b.Image,
                                                       ...
                                                       b.NumPages, b.Format, b.Description,
        ...
        public async Task<BookListDto[]> GetSimilarBookAsync(int bookId, int count)
        {
            ...
            var similarBooks = await context.GenreBooks.Where(gb => gb.BookId == bookId)
                                ...
                                .Select(gb => gb.Book)
                                .Where(b => b.Id != bookId) // пропустите эту книгу из раздела похожие книги
                                .Select(b => new BookListDto(b.Id, b.Title, b.Image, new AuthorDto(b.Author.Name, b.Author.Slug)))
                                .OrderBy(b => Guid.NewGuid())
                                .Take(count)
                                .ToArrayAsync();
